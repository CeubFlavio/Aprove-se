...
export default FilteredSimulateScreen = ({navigation}) => {
    ...
    async function getQuestion(){
        setLoading(true);
        let stringIds = user.exceptionCategory.toString().replaceAll(",", ";");
        api.get(`/get-random-question?ids=${stringIds}`, {headers: { 'Authorization': `Bearer ${user.token}`}})
            .then(response => {
                setLoading(false);
    ...
    return (
        <MainContainer>
            <HeaderContainer>
                ...
                <Title>Matérias Filtradas</Title>
            </HeaderContainer>
